...
// Options
  ...
  // if the state is RED and the next attempt is scheduled before the timeout expires throw an Error and abort. We do not allow the request to go upstream.
  // If the state is RED but the timeout period expired, we want to switch state to YELLOW and allow the request to pass.
  // If the state is NOT RED we try to make the request, and based on whether the request succeeded or failed, we call the appropriate handler method
